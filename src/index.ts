...
app.post('/api/v1/order', (req, res) => {
    ...
    const {baseAssest, quoteAssest, price, quantity, side, kind} = order.data;
    const orderId = getOrderId(); 

    if(baseAssest != BASE_ASSET || quoteAssest != QUOTE_ASSET){
...
function fillOrder(orderId: string, price: number, quantity: number, side: "buy" | "sell", type?: "ioc"):{
    ...
    if(side === "buy"){
         ...
         orderbook.asks.forEach(o => {
            if (o.price <= price && quantity > 0) {
                ...
                bookWithQuantity.asks[o.price] = (bookWithQuantity.asks[o.price] || 0) - fillquantity;
                fills.push({
                    price: o.price,
                    qty: fillquantity,
